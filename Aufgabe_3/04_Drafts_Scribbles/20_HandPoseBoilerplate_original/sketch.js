...
function draw() {
  ...
  if (isModelReady) {
    ...
    if (hands.length > 0) {
      ...
      // Zugriff auf die Daumenspitze (Keypoint 4)
      let daumenspitze = hand.keypoints[4];
      let x = daumenspitze.x * ratio;
      let y = daumenspitze.y * ratio;

      // Beispiel: Zeichne einen großen Kreis an der Daumenspitze
      fill(255, 0, 0);
      circle(x, y, 30);

      // Zugriff auf die Zeigefingerspitze (Keypoint 8)
      let Zeigefinger = hand.keypoints[8];
      x = Zeigefinger.x * ratio;
      y = Zeigefinger.y * ratio;
