...
function draw() {
  ...
  if (isModelReady) {
    drawHandPoints();
if (hands.length > 0) {
  const hand = hands[0];                // erste erkannte Hand
  const thumbTip = hand.keypoints[4];   // Daumen-Tip
  const indexTip = hand.keypoints[8];   // Zeigefinger-Tip

  console.log('Daumen-Tip (video coords):', thumbTip.x, thumbTip.y);
  console.log('Zeigefinger-Tip (video coords):', indexTip.x, indexTip.y);

  if (thumbTip && indexTip) {
  // Mit ratio in Canvas-Koordinaten umrechnen
  const cx = (thumbTip.x + indexTip.x) / 2 * ratio;
  const cy = (thumbTip.y + indexTip.y) / 2 * ratio;

  // Abstand der Tips als Durchmesser (video coords → mit ratio skalieren)
  const d = dist(thumbTip.x, thumbTip.y, indexTip.x, indexTip.y) * ratio;

  // Lila Kreis, kein Stroke
  
  circle(cx, cy, d);
}
}
// Hole Daumen- und Zeigefinger-Tips (ml5 indices 4 und 8)
  }
